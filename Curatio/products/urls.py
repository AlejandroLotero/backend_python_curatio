...
urlpatterns = [
    ...
    # =========================
    path(
        "v1/inventory/medications/",
        api_views.medications_resource,
        name="medications_resource",
    ),
    path(
        "v1/inventory/medications/<int:medication_id>/",
        api_views.medication_detail_resource,
        name="medication_detail_resource",
    ),
    path(
        "v1/inventory/medications/<int:medication_id>/status/",
        api_views.medication_status_resource,
        name="medication_status_resource",
    ),

    path(
        "v1/catalogs/pharmaceutical-forms/",
        api_views.pharmaceutical_forms_catalog,
        name="pharmaceutical_forms_catalog",
    ),
    path(
        "v1/catalogs/presentations/",
        api_views.presentations_catalog,
        name="presentations_catalog",
    ),
    path(
        "v1/catalogs/administration-routes/",
        api_views.administration_routes_catalog,
        name="administration_routes_catalog",
    ),
    path(
        "v1/catalogs/laboratories/",
        api_views.laboratories_catalog,
        name="laboratories_catalog",
    ),
    path(
        "v1/catalogs/medication-statuses/",
        api_views.medication_statuses_catalog,
        name="medication_statuses_catalog",
    ),

    # =========================
    # PROVEEDORES
    # =========================
    # Listado y creación de proveedores para la SPA.
    path(
        "v1/procurement/suppliers/",
        api_views.suppliers_catalog,
        name="suppliers_catalog",
    ),

    # Detalle por NIT.
    # GET  -> consulta read-only del proveedor
    # PATCH -> actualización completa de campos permitidos
    path(
        "v1/procurement/suppliers/<str:supplier_nit>/",
        api_views.supplier_detail_resource,
        name="supplier_detail_resource",
    ),

    # Cambio específico de estado Activo/Inactivo.
    path(
        "v1/procurement/suppliers/<str:supplier_nit>/status/",
        api_views.supplier_status_resource,
        name="supplier_status_resource",
    ),
]